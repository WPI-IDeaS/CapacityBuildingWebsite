...
function defBubble(tag) {
  return (
    <span className='definition'>
      <svg width="50px" className='def-styled'>
        <polygon points="0,75 50,150 50,0"/>
      </svg>
      <span className='def-box'>
        <div className='def-blurb'>
          {defMap[tag.toUpperCase()]}
        </div>
        {/*<svg height="150" width="250" className='def-styled'>
          <polygon points="0,150 250,150 250,0 0,0"/>
        </svg>*/}
      </span>
    </span>
  );
...
function makeDef(text, tag) {
  return (
    <span>
      <span className='definable' onMouseEnter={defHover} onMouseLeave={defUnHover}>
        {text}
      </span>
      {defBubble(tag)}
    </span>
